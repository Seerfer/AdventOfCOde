...
class Queues:
    ...
    def execute_move_instruction(
        self, instruction: str, multiple_el: bool = False
    ) -> None:
        template = "move (?P<number>.*) from (?P<q1>.*) to (?P<q2>.*)"
        parsed = self.read_values_from_string_template(instruction, template)
        if multiple_el:
            popped = self.__queues[int(parsed.get("q1"))].pop_multiple_elements(
                int(parsed.get("number"))
            )
